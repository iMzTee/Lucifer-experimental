...
class QRSVGLWidget(QtOpenGL.QGLWidget):
    ...
    def render(self, total_time, delta_time, width, height):
        ...
        # ── Boost meter (bottom-right, circular arc like Rocket League) ──
        if self.spectate_idx > -1 and self.spectate_idx < len(state.car_states):
            car_state = state.car_states[self.spectate_idx]
            boost_frac = car_state.boost_amount / 100.0
            self._draw_boost_meter(width, height, boost_frac, car_state.team_num)
        ...
        boost_text = "Boost: {:.0f}".format(car_state.boost_amount) if (self.spectate_idx > -1 and self.spectate_idx < len(state.car_states)) else ""
        if boost_text:
            ui_text += boost_text + "\n"
    ...
    def _draw_boost_meter(self, win_width, win_height, boost_frac, team_num):
        """Draw a circular boost meter in the bottom-right corner."""
        self.ctx.disable(moderngl.DEPTH_TEST)
        self.ctx.disable(moderngl.CULL_FACE)

        # Meter parameters
        cx = win_width - 90   # center X (from right edge)
        cy = win_height - 90  # center Y (from bottom)
        outer_r = 55
        inner_r = 40
        segments = 48
        arc_start = math.pi * 0.75   # start at bottom-left of circle
        arc_total = math.pi * 1.5    # 270 degree arc

        # Switch to 2D orthographic projection
        proj_2d = Matrix44.orthogonal_projection(0, win_width, win_height, 0, -1, 1)
        self.pr_m_vp.write(proj_2d.astype('f4'))
        self.pr_m_model.write(Matrix44.identity().astype('f4'))

        # Background arc (dark)
        bg_verts = []
        for i in range(segments + 1):
            angle = arc_start + arc_total * (i / segments)
            bg_verts.append([cx + math.cos(angle) * outer_r, cy - math.sin(angle) * outer_r, 0])
            bg_verts.append([cx + math.cos(angle) * inner_r, cy - math.sin(angle) * inner_r, 0])

        bg_data = np.array(bg_verts, dtype='f4')
        self.lines_vbo.write(bg_data, 0)
        self.pr_global_color.write(Vector4((0.15, 0.15, 0.15, 0.8)).astype('f4'))
        self.t_none.use()
        self.ctx.screen.use()
        self.vaos['render_lines'].render(moderngl.TRIANGLE_STRIP, vertices=len(bg_verts))

        # Filled arc (boost amount)
        if boost_frac > 0.005:
            fill_segments = max(1, int(segments * boost_frac))
            fill_verts = []
            for i in range(fill_segments + 1):
                angle = arc_start + arc_total * (i / segments)
                fill_verts.append([cx + math.cos(angle) * outer_r, cy - math.sin(angle) * outer_r, 0])
                fill_verts.append([cx + math.cos(angle) * inner_r, cy - math.sin(angle) * inner_r, 0])

            fill_data = np.array(fill_verts, dtype='f4')
            self.lines_vbo.write(fill_data, 0)

            # Color: orange for orange team, blue-cyan for blue team
            if team_num == 0:
                color = Vector4((0.1, 0.7, 1.0, 0.95))  # blue
            else:
                color = Vector4((1.0, 0.6, 0.1, 0.95))   # orange
            self.pr_global_color.write(color.astype('f4'))
            self.ctx.screen.use()
            self.vaos['render_lines'].render(moderngl.TRIANGLE_STRIP, vertices=len(fill_verts))

        # Boost number in center (rendered as text in UI bar instead)

        self.pr_global_color.write(Vector4((0, 0, 0, 0)).astype('f4'))
        self.ctx.enable(moderngl.DEPTH_TEST)
        self.ctx.enable(moderngl.CULL_FACE)
